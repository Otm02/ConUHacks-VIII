...
class Schedule:
    ...
    def print_stats(self):
        ...
        #     print("\n")
        print("Total Revenue: ", sum([day.revenue for day in self.days]), "$")
        print("Total Loss: ", sum([day.loss for day in self.days]), "$")
        print(
            "Total compact serviced: ",
            sum([day.service_info["compact"]["serviced"] for day in self.days]),
        )
        print(
            "Total medium serviced: ",
            sum([day.service_info["medium"]["serviced"] for day in self.days]),
        )
        print(
            "Total full-size serviced: ",
            sum([day.service_info["full-size"]["serviced"] for day in self.days]),
        )
        print(
            "Total class 1 truck serviced: ",
            sum([day.service_info["class 1 truck"]["serviced"] for day in self.days]),
        )
        print(
            "Total class 2 truck serviced: ",
            sum([day.service_info["class 2 truck"]["serviced"] for day in self.days]),
        )
        print(
            "Total compact rejected: ",
            sum([day.service_info["compact"]["rejected"] for day in self.days]),
        )
        print(
            "Total medium rejected: ",
            sum([day.service_info["medium"]["rejected"] for day in self.days]),
        )
        print(
            "Total full-size rejected: ",
            sum([day.service_info["full-size"]["rejected"] for day in self.days]),
        )
        print(
            "Total class 1 truck rejected: ",
            sum([day.service_info["class 1 truck"]["rejected"] for day in self.days]),
        )
        print(
            "Total class 2 truck rejected: ",
            sum([day.service_info["class 2 truck"]["rejected"] for day in self.days]),
        )
